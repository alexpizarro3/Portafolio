...
const ProjectCard = React.memo(({ project, isActive, onHover, onClick }: Props) => (
  <motion.div
    ...
    transition={{ type: 'spring', damping: 20, stiffness: 100, delay: project.id * 0.08, duration: 0.8 }}
    viewport={{ once: true,  amount: 0.2 }}
    className={`relative flex flex-col md:flex-row items-center gap-6 cursor-pointer transition-all duration-500 rounded-xl p-2 ${
      isActive
        ? 'scale-100 bg-white dark:bg-gray-900 shadow-xl z-20'
        : 'scale-75 opacity-60 hover:scale-80 hover:opacity-100 bg-gray-100 dark:bg-gray-800 z-10'
    }`}
  >
    {isActive && (
    ...
    <div className="w-full md:w-3/5 flex justify-center">
      ...
      >
        <Image
          ...
          width={600}
          height={375}
          className={`rounded-xl w-full h-auto ${project.id === 6 ? 'object-contain' : 'object-cover'}`}
        />
    ...
    <div className="w-full md:w-2/5 text-center md:text-left transition-all duration-500">
